...
moveBall:
	...
	jr z, ball_move_tr			;			ball_moveTopRight()
	cmp A,DIR_BTM_RIGHT		;		case "bottom right"
	jr z, ball_move_br			;			ball.moveBottomRight()
	cmp A,DIR_BTM_LEFT		;		case "bottom left"
	jr z, ball_move_bl			;			ball.moveBottomLeft()
	jr ball_move_tl			;		else => ball.moveTopLeft()
	...
	ball_checkForCollisions_br:
		ball_check_br_y:
			...
			ld B,(ballYPos)
			cmp A,B
			jr c, ball_check_br_x		;	if pos.y < y_min
			ld A,DIR_TOP_RIGHT		;		direction = "top right"
			ld (ballDir),A
		ball_check_br_x:
			ld A,(ballXPos)
			cmp A,BALL_P2_CONTACT		;	if not pos.x < p2.pos.x
			jr c, end_ball_check_br_x		;	
			ld A,(ballYPos)
			ld B,(p2Position)			;	if not pos.y < p2.pos.y
			cmp A,B						;	
			jr c, end_ball_check_br_x
